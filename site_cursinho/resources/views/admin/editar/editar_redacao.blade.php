...
@if(auth()->user()->nivel === 'admin')
    ...
    <div class="logo">
        ...
        <div class="container">
            ...
            @foreach($rows as $row)
            <form action="{{ route('redacao.update',  urlencode($row->titulo)) }}" method="post" enctype='multipart/form-data'>
            @method('PUT')
            @csrf
...
<div class="form-row">
            ...
            @component('components.card_form')
                ...
                @slot('required')
                    value="{{isset($row->proposta_arquivo) ? $row->proposta_arquivo : ''}}"
                @endslot
