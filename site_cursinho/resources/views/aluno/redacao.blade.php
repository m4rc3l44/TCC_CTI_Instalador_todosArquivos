...
<!-- </head>
    ...
    <main>
        <section class="container_main">
            ...
            <div class="card">
            <a href="{{ route('redacao_aluno.edit', urlencode($row->titulo)) }}" class="btnRed">   
              <img src="{{asset($row->nome_imagem)}}" alt="Imagem" class="teste_imagem">
              <div class="containercard">
                  <center>
                    <h4>{{$row->titulo}}</h4>
                    <p alt="{{$row->texto_imagem}}">{{$row->descricao}}</p>
                    <a href="{{asset($row->proposta_arquivo)}}" download style='text-decoration:none;'><ion-icon name="download-outline" ></ion-icon>&nbsp;&nbsp;Baixar Materiais</a>
                  </center>
              </div>
            </a>
            </div> 
