...
@if(auth()->user()->nivel === 'admin')
    ...
    <div class="mamae">
        ...
        <div class="container">
            ...
            @foreach($rows as $row)
            <!-- auth()->user()->cpf <br> {{ $row->fk_cpf_aluno}} -->
            @if(auth()->user()->cpf != $row->fk_cpf_aluno)
                        <?php
                            header("Location: " . route('home'));
                            exit; 
                        ?>   
            @endif 
            <form action="{{ route('admin.redacao_enviada.update', ['id_redacao' => $row->id_redacao, 'fk_cpf_aluno'=> auth()->user()->cpf]) }}" method="post" enctype='multipart/form-data'>
            @method('PUT')
            @csrf
